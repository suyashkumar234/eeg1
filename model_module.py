...
def apply_eeg_augmentations(eeg_data, aug_prob=0.5, selective_only=False):
    """
    Apply EEG-specific augmentations (Strategy 1: Simple augmentation during training)
    
    Args:
        eeg_data: [batch, channels, time_steps] - EEG data tensor
        aug_prob: Probability of applying each individual augmentation
        selective_only: If True, only apply time shift and noise (gentler augmentations)
    
    Returns:
        augmented_data: Augmented EEG data with same shape
        applied_augs: List of applied augmentation types for statistics
    """
    augmented_data = eeg_data.clone()  # Don't modify original data
    batch_size, channels, time_steps = eeg_data.shape
    device = eeg_data.device
    applied_augs = []
    
    # Augmentation 1: Time Shifting (Most important for temporal attention)
    if random.random() < aug_prob:
        # Shift by ±0.1 seconds (±12-13 samples at 128Hz, ±25-26 samples at 256Hz)
        max_shift = min(15, time_steps // 10)  # Adaptive to sequence length
        shift_samples = random.randint(-max_shift, max_shift)
        augmented_data = torch.roll(augmented_data, shifts=shift_samples, dims=-1)
        applied_augs.append('time_shift')
    
    # Augmentation 2: Small Gaussian Noise (Improves robustness)
    if random.random() < aug_prob:
        # Add 1% noise relative to signal standard deviation
        noise_std = 0.01 * eeg_data.std()
        noise = torch.randn_like(augmented_data) * noise_std
        augmented_data = augmented_data + noise
        applied_augs.append('noise')
    
    # Only apply aggressive augmentations if not in selective mode
    if not selective_only:
        # Augmentation 3: Time Masking (Forces model to use all temporal information)
        if random.random() < aug_prob:
            # Mask 5-15 consecutive time steps
            mask_length = random.randint(5, min(15, time_steps // 10))
            start_idx = random.randint(0, time_steps - mask_length)
            augmented_data[:, :, start_idx:start_idx+mask_length] = 0
            applied_augs.append('time_mask')
        
        # Augmentation 4: Channel Dropout (Simulates bad electrodes)
        if random.random() < aug_prob:
            # Randomly drop 1-2 channels
            num_dropout = random.randint(1, min(2, channels // 8))
            dropout_channels = random.sample(range(channels), num_dropout)
            augmented_data[:, dropout_channels, :] = 0
            applied_augs.append('channel_dropout')
        
        # Augmentation 5: Amplitude Scaling (Per-channel variability)
        if random.random() < aug_prob:
            # Scale each channel by factor between 0.9-1.1
            scale_factors = torch.empty(batch_size, channels, 1).uniform_(0.9, 1.1).to(device)
            augmented_data = augmented_data * scale_factors
            applied_augs.append('amplitude_scale')
    
    return augmented_data, applied_augs
...
class DARNet(nn.Module):
    def __init__(self, args):
        ...
        self.use_augmentation = getattr(args, 'use_augmentation', True)
        self.augmentation_prob = getattr(args, 'augmentation_prob', 0.6)
        self.aug_individual_prob = getattr(args, 'aug_individual_prob', 0.5)
