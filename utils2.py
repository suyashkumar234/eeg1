...
def full_darnet_reconstruction_loss(reconstructed, original, mask_info_batch, device, use_patch_masking=True):
    """Unified reconstruction loss for full DARNet SSL approach"""
    if use_patch_masking:
        return patch_reconstruction_loss(reconstructed, original, mask_info_batch, device)
    else:
        # Original temporal masking loss (backward compatibility)
        batch_size = reconstructed.shape[0]
        total_loss = 0
        
        for i in range(batch_size):
            mask_indices = mask_info_batch[i].to(device)
