...
from model_module2 import DARNet
...
class SSLTrainer():
    ...
    def ssl_step(self):
        ...
        for i_batch, batch_data in enumerate(self.ssl_loader):
            ...
            # Create patch-based masked version (15% for comparison with temporal masking)
            masked_eeg, mask_info_batch = create_masked_batch(
                eeg_data, 
                mask_ratio=0.15,  # Same as temporal masking for fair comparison
            ...
            # Always use full_darnet loss (since U-Net showed bad results)
            ssl_loss = full_darnet_reconstruction_loss(
                reconstructed, eeg_data, mask_info_batch, device, use_patch_masking=True
            )
...
def ssl_pretrain(args, train_ids, worker_init_fn):
    ...
    print("=" * 80)
    print("SSL PRE-TRAINING PHASE")
    print("=" * 80)
    
    # Load SSL data (training subjects only, no labels) with patch-based masking
    ssl_data = getSSLData(args, train_ids, mask_ratio=0.15, use_patch_masking=True)
    ...
    ssl_loader = DataLoader(dataset=ssl_dataset, batch_size=args.batch_size, shuffle=True, drop_last=True,
                           worker_init_fn=worker_init_fn, num_workers=8)  # Deterministic multi-worker
...
if __name__ == '__main__':
    ...
    # data
    args.dataset = 'MM-AAD'
    args.start_time = datetime.now().strftime(f"task1_AAD_{args.dataset}_original_ssl2%Y-%m-%d-%H-%M-%S")
    ...
    args.ssl_approach = "full_darnet"  # "full_darnet" or "unet"
    if args.ssl_enabled:
        if args.ssl_approach == "full_darnet":
            args.model = "DARNet_SSL_FullDARNet"
        else:
            args.model = "DARNet_SSL_UNet"
    else:
        args.model = "DARNet"
    args.batch_size = 128
    args.lr = 5e-4
