...
from model_module import DARNet, contrastive_loss, apply_eeg_augmentations
...
class Trynetwork():
    def __init__(self, model, train_loader, valid_loader, test_loader, batch_size, lr, weight_decay, worker_init_fn, args):
        ...
        self.aug_stats = get_augmentation_statistics()
    ...
    def train_step(self):
        ...
        for i_batch, batch_data in enumerate(self.datasets['train']):
            ...
            # Strategy 1: Apply augmentation during training with configurable probability
            self.aug_stats['total_batches'] += 1
            if random.random() < self.args.augmentation_prob:
                augmented_data, applied_augs = apply_eeg_augmentations(
                    train_data, 
                    aug_prob=self.args.aug_individual_prob,
                    selective_only=getattr(self.args, 'selective_augmentation', False)
                )
                train_data = augmented_data
                self.aug_stats['augmented_batches'] += 1
                
                # Track individual augmentation types
                for aug_type in applied_augs:
                    if aug_type == 'time_shift':
                        self.aug_stats['time_shift_count'] += 1
                    elif aug_type == 'noise':
                        self.aug_stats['noise_count'] += 1
                    elif aug_type == 'time_mask':
                        self.aug_stats['time_mask_count'] += 1
                    elif aug_type == 'channel_dropout':
                        self.aug_stats['channel_dropout_count'] += 1
                    elif aug_type == 'amplitude_scale':
                        self.aug_stats['amplitude_scale_count'] += 1
    ...
    def test_batch(self, input, label):
        ...
        with torch.no_grad():
            ...
            # Model returns only attention logits during evaluation (no augmentation)
    ...
    def evaluate_step(self, Flag_test):
        ...
        with torch.no_grad():
            ...
            for i_batch, batch_data in enumerate(self.datasets[setname]):
                ...
                # No augmentation during validation/testing - clean data only
    ...
    def train(self, args, testsub_id):
        ...
        for epoch in range(1, args.max_epoch + 1):
            ...
            # Add augmentation statistics every 10 epochs
            if epoch % 10 == 0:
                log_augmentation_stats(self.aug_stats, epoch)
        ...
        print("="*50)
        log_augmentation_stats(self.aug_stats, "Final")
...
if __name__ == '__main__':
    ...
    # data
    args.dataset = 'MM-AAD'
    args.start_time = datetime.now().strftime(f"task1_AAD_{args.dataset}_contra_aug_%Y-%m-%d-%H-%M-%S")
    ...
    # basic info of the model - WINNING HYPERPARAMETERS (Set 2: High Contrastive Focus)
    args.optimizer='SGD'
    args.batch_size = 64          # Winning configuration
    args.lr = 1e-2
    args.lam = 0.2
    ...
    args.lambda_contrastive = 0.8   # Winning contrastive learning weight
    args.temperature = 0.07         # Winning InfoNCE temperature parameter
    

    args.use_augmentation = True         # Enable augmentation
    args.augmentation_prob = 0.3         # 30% probability of applying augmentation to a batch (reduced from 60%)
    args.aug_individual_prob = 0.4       # 40% probability for each individual augmentation type (reduced from 50%)
    args.selective_augmentation = True   # Only use time shift and noise (skip aggressive augmentations)
    ...
    if args.use_augmentation:
        if args.selective_augmentation:
            model_components.append("AugGentle") 
        else:
            model_components.append("Aug")  # Full augmentation
    ...
    print('=' * 108)
    print("🎯 IMPROVED AUGMENTATION EXPERIMENT RESULTS:")
    print(f"   - Baseline (no aug): 55.18% (from previous Set 2 results)")
    print(f"   - Previous aggressive aug: 53.84% (-1.34%)")
    print(f"   - Current gentle aug: {mean_acc:.2f}%")
    
    print("📊 Augmentation Strategy:")
    print(f"   - Batch probability: {args.augmentation_prob*100:.0f}% (vs 60% before)")
    print(f"   - Individual probability: {args.aug_individual_prob*100:.0f}% (vs 50% before)")
    print(f"   - Selective mode: {'Yes' if args.selective_augmentation else 'No'} (only time shift + noise)")
    print('=' * 108)
