...
class SSLTrainer():
    ...
    def ssl_step(self):
        ...
        for i_batch, batch_data in enumerate(self.ssl_loader):
            ...
            # Choose loss function based on SSL approach
            if hasattr(self.model, 'ssl_approach') and self.model.ssl_approach == 'full_darnet':
                ssl_loss = full_darnet_reconstruction_loss(reconstructed, eeg_data, mask_indices_batch, device)
            else:
                ssl_loss = unet_reconstruction_loss(reconstructed, eeg_data, mask_indices_batch, device)
...
def ssl_pretrain(args, train_ids, worker_init_fn):
    ...
    # Load SSL data (training subjects only, no labels)
    ssl_data = getSSLData(args, train_ids, mask_ratio=0.15)
...
if __name__ == '__main__':
    ...
    # data
    args.dataset = 'MM-AAD'
    args.start_time = datetime.now().strftime(f"task1_AAD_{args.dataset}_original_ssl_%Y-%m-%d-%H-%M-%S")
